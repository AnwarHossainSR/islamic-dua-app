...
export const challengesApi = {
    ...
    return data.map((challenge) => {
      ...
      const completionPercentage =
        progress?.total_completed_days && challenge.total_days
          ? Math.min(Math.round((progress.total_completed_days / challenge.total_days) * 100), 100)
          : 0
  ...
  getById: async (id: string) => {
    try {
      ...
      apiLogger.error('Get challenge by ID failed', { id, error: error.message })
      throw error
    }
  },
  ...
  start: async (userId: string, challengeId: string) => {
    try {
      ...
      if (error) {
        apiLogger.error('Start challenge failed', { userId, challengeId, error: error.message })
      ...
      apiLogger.error('Start challenge exception', { userId, challengeId, error: error.message })
      throw error
    }
  },
  ...
  getProgress: async (progressId: string) => {
    try {
      ...
      apiLogger.error('Get progress failed', { progressId, error: error.message })
      throw error
    }
  },
  ...
  restart: async (progressId: string, challengeId: string) => {
    try {
      ...
      apiLogger.error('Restart challenge failed', { progressId, challengeId, error: error.message })
      throw error
    }
  },
