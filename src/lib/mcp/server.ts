...
export class EnhancedMCPServer {
    ...
    return [
      {
        ...
        columns: [
          ...
          { name: 'description_bn', type: 'string', description: 'Challenge description in Bengali' },
      ...
      },
      {
        ...
        columns: [
          ...
          { name: 'user_id', type: 'string', description: 'User ID (always filtered by current user)' },
  ...
  async executeFunction(name: string, args: any = {}) {
    ...
    try {
      ...
      apiLogger.error('MCP function execution failed', { function: name, args, error: error.message })
      throw error
    }
  }
  ...
  private async queryUserChallenges(args: {
    ...
    if (error) {
      const { apiLogger } = await import('@/lib/logger')
      apiLogger.error('Query user challenges failed', { userId: this.userId, args, error: error.message })
      throw error
    }
  ...
  private async queryDuas(args: {
    ...
    if (args.searchText) {
      query = query.or(`title_bn.ilike.%${args.searchText}%,title_en.ilike.%${args.searchText}%,translation_bn.ilike.%${args.searchText}%`)
    }
  ...
  private async getChallengeStatistics(args: { challengeId?: string }) {
    if (args.challengeId) {
      ...
      if (error) {
        const { apiLogger } = await import('@/lib/logger')
        apiLogger.error('Get challenge statistics failed', { userId: this.userId, challengeId: args.challengeId, error: error.message })
        throw error
      }
      ...
      return {
        ...
        completionRate: data.current_day > 0
    ...
    if (error) {
      const { apiLogger } = await import('@/lib/logger')
      apiLogger.error('Get overall statistics failed', { userId: this.userId, error: error.message })
      throw error
    }
    ...
    const totalCompletedDays = allProgress.reduce((sum, p) => sum + (p.total_completed_days || 0), 0)
    const averageStreak = allProgress.reduce((sum, p) => sum + (p.current_streak || 0), 0) / totalChallenges || 0
    ...
    return [
      {
        ...
        description: 'Get the complete database schema to understand what data is available. Use this first to plan your queries.',
        parameters: {},
      },
      {
        name: 'query_user_challenges',
        description: 'Query user challenges with flexible filtering - filter by status (active/paused/completed), minimum streak',
        parameters: {
          status: {
            type: 'string',
      ...
      },
      {
        ...
        description: 'Analyze all user streaks - shows best performers and challenges needing attention',
        parameters: {},
      },
      {
