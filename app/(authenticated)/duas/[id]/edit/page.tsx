...
import { getDuaById, updateDua } from '@/lib/actions/duas'
...
export default function EditDuaPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [duaId, setDuaId] = useState<string>('')
  ...
  useEffect(() => {
    const loadDua = async () => {
      ...
      try {
        ...
        toast({
          title: 'Error',
          description: 'Failed to load dua',
          variant: 'destructive',
        })
        router.push('/duas')
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateDua(duaId, formData)
      toast({
        title: 'Success',
        description: 'Dua updated successfully',
        variant: 'default',
      })
      router.push(`/duas/${duaId}`)
    } catch (error: any) {
      toast({
        title: 'Error',
        description: error.message || 'Failed to update dua',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
