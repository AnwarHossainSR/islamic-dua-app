...
export const settingsApi = {
  ...
  async getDbStats() {
    ...
    const totalRecords = (duas.count || 0) + (challenges.count || 0) + (progress.count || 0) + (admins.count || 0);
  ...
  async createBackup(storeInSupabase: boolean) {
    const tables = [
      'permissions', 'user_roles', 'admin_users', 'app_settings', 'user_settings',
      'dua_categories', 'duas', 'activity_stats', 'challenge_templates',
      'role_permissions', 'user_activity_stats', 'challenge_activity_mapping',
      'user_challenge_progress', 'user_challenge_daily_logs', 'user_missed_challenges',
      'notifications', 'api_logs'
    ];
  ...
  async getCredentials() {
    const { data, error } = await supabase.from('webauthn_credentials').select('*').order('created_at', { ascending: false });
    if (error) throw error;
    return data || [];
  },

  async deleteCredential(credentialId: string) {
    const { error } = await supabase.from('webauthn_credentials').delete().eq('credential_id', credentialId);
    if (error) throw error;
    return { success: true };
  },
};
