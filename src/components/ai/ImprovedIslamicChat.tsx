...
export function ImprovedIslamicChat({ initialSessions, hasOpenAIKey }: ImprovedIslamicChatProps) {
  ...
  const loadMessages = async (sessionId: string) => {
    try {
      ...
      apiLogger.error('Load messages failed', { sessionId, error: error.message })
    }
  }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (chatMode === 'database') {
        ...
        response = await EnhancedAIService.askGeneralQuestion(messageText)
      }
      ...
      const { apiLogger } = await import('@/lib/logger')
      apiLogger.error('Chat send message failed', { sessionId: session.id, error: error.message })
