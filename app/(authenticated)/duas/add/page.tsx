...
import { createDua } from '@/lib/actions/duas'
...
export default function AddDuaPage() {
  ...
  const loadFromJson = () => {
    try {
      ...
      toast({
        title: 'JSON Loaded',
        description: 'Data has been loaded from JSON input.',
      })
    } catch (error) {
      toast({
        title: 'Invalid JSON',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Dua Created',
        description: 'The new dua has been created successfully.',
      })
      router.push('/duas')
    } catch (error: any) {
      toast({
        title: 'Error',
        description: error.message || 'Failed to create dua',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Add New Dua</h1>
